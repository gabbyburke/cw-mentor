...
def test_streaming():
    """
    Test the streaming analysis function.
    """
    # Hardcoded test data
    transcript = [
        {
    ...
    print(f"Testing streaming response from {url}...")
    print("=" * 50)
    ...
    try:
        ...
        for chunk in response.iter_content(chunk_size=None, decode_unicode=True):
            if chunk:
                ...
                # Check if we've received the complete analysis
                if "[ANALYSIS_COMPLETE]" in full_response and not analysis_json:
                    # Extract the JSON after the marker
                    marker = "[ANALYSIS_COMPLETE]\n"
                    start = full_response.find(marker) + len(marker)
                    # Find end (either citations marker or end of response)
                    citations_marker = "[CITATIONS_COMPLETE]\n"
                    end = full_response.find(citations_marker)
                    if end == -1:
                        end = len(full_response)
                    
                    json_text = full_response[start:end].strip()
                    try:
                        analysis_json = json.loads(json_text)
                        print("\n\n" + "=" * 50)
                        print("ANALYSIS PARSED SUCCESSFULLY:")
                        print(f"Citations in analysis: {len(analysis_json.get('citations', []))}")
                    except json.JSONDecodeError as e:
                        print(f"\nError parsing analysis JSON: {e}")
                
                # Check if we've received the citations
                if "[CITATIONS_COMPLETE]" in full_response and not citations_json:
                    # Extract the citations JSON
                    marker = "[CITATIONS_COMPLETE]\n"
                    start = full_response.find(marker) + len(marker)
                    json_text = full_response[start:].strip()
                    
                    try:
                        citations_json = json.loads(json_text)
                        print("\n\n" + "=" * 50)
                        print(f"CITATIONS RECEIVED: {len(citations_json.get('citations', []))} items")
                        
                        # Print first few citations as examples
                        for i, citation in enumerate(citations_json.get('citations', [])[:3]):
                            print(f"\nCitation {i+1}:")
                            print(f"  Source: {citation.get('source', 'N/A')}")
                            print(f"  Pages: {citation.get('pages', 'N/A')}")
                            print(f"  Text preview: {citation.get('text', '')[:100]}...")
                    except json.JSONDecodeError as e:
                        print(f"\nError parsing citations JSON: {e}")
        
        print("\n\n" + "=" * 50)
        print(f"Total response length: {len(full_response)} characters")
        print(f"Analysis received: {'Yes' if analysis_json else 'No'}")
        print(f"Citations received: {'Yes' if citations_json else 'No'}")
